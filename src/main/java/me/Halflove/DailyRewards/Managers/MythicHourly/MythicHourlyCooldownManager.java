...
public class MythicHourlyCooldownManager {
    public static boolean getAllowRewardip(Player p) {
        ...
        if (MythicHourlySettingsManager.getConfig().getBoolean("mysql.enabled")) {
            ...
            millis = MythicHourlySettingsManager.getData().getLong(String.valueOf(ip) + ".millis");
        }
    ...
    public static boolean getAllowRewardUUID(Player p) {
        ...
        if (MythicHourlySettingsManager.getConfig().getBoolean("mysql.enabled")) {
            ...
            millis = MythicHourlySettingsManager.getData().getLong(p.getUniqueId() + ".millis");
        }
        return (current > millis);
    }
    ...
    public static long getStreakUUID(Player player) {
        return MythicHourlySettingsManager.getData().getLong(player.getUniqueId() + ".reset");
    }

    public static long getStreakIP(Player player) {
        String ip = player.getAddress().getAddress().getHostAddress();
        ip = ip.replace(".", "-");
        return MythicHourlySettingsManager.getData().getLong(String.valueOf(ip) + ".reset");
    }

    public static long getTime(Player player) {
        return MythicHourlySettingsManager.getData().getLong(player.getUniqueId() + ".millis");
    }

    public static long getTimeip(Player player) {
        String ip = player.getAddress().getAddress().getHostAddress();
        ip = ip.replace(".", "-");
        return MythicHourlySettingsManager.getData().getLong(String.valueOf(ip) + ".millis");
    }
